...
(defun dotspacemacs/user-config ()
 ...
 ;; (define-key Jane.fe-mode-map (kbd "<escape>") 'ignore)

 ;; Quarter window scrolling (<C-u> <C-d> do half-window)
 ...
 (defun scroll-up-half ()
   ...
   (move-to-window-line (window-quarter-height))
   )

 (defun scroll-down-half ()
   (interactive)
   (scroll-down (window-quarter-height))
   (move-to-window-line (- count-screen-lines window-quarter-height))
   )

 (global-set-key [next] 'scroll-up-half)
 (global-set-key [prior] 'scroll-down-half)

 (spacemacs/set-leader-keys-for-major-mode 'rust-mode "ht" 'racer-describe)
 ...
 ;; open gnome-terminal at local folder
 (defun open-gnome-terminal ()
  ...
  ;; Start with clear ivy search rather than the "^"
  ...
  ;; (set-variable 'ycmd-server-command '("python3" "/home/viraj/dev/ycmd/ycmd"))
  ...
  ;; ;; this weirdly changes the buffer position...
  ;; (add-hook 'rust-mode-hook
  ;;           (lambda ()
  ;;             (add-hook 'after-save-hook 'cargo-process-check nil 'make-it-local)))
 )
